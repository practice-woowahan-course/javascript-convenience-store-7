...
class InputView {
  ...
  static async inputPromotionFreeApply(name, quantity) {
    const inputPromotionApply = await MissionUtils.Console.readLineAsync(
      `현재 ${name}은(는) ${quantity}개를 무료로 더 받을 수 있습니다. 추가하시겠습니까? (Y/N)`
    )
    return inputPromotionApply;
  }

  static async inputPromotionGiveUp(name, amount) {
    const inputMembershipDiscount = await MissionUtils.Console.readLineAsync(
      `현재 ${name} ${amount}개는 프로모션 할인이 적용되지 않습니다. 그래도 구매하시겠습니까? (Y/N)`
    )
    return inputMembershipDiscount;
  }
